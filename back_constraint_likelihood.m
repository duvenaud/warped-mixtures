...
function [ f, grad_X, log_hypers_grad ] = back_constraint_likelihood( X, Y, log_hypers )
    % Returns the likelihood of the back_constraints, along with 
    ...
    %K2 = exp(log_hypers.alpha) * exp(K1) ;
    %K2 = exp(log_hypers.alpha + K1) ;
    %K = K2 + eye(N)*max(exp(log_hypers.betainv), 1e-6);  % HACK
    ...
    log_hypers_grad.alpha = 0;%-sum(sum(gradLK.*K2));
    log_hypers_grad.betainv = 0;%;-exp(log_hypers.betainv) * trace(gradLK) ;
    log_hypers_grad.gamma  = 0;%-sum(sum(gradLK.*(K1.*K2)));
