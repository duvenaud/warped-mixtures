...
for i = 1:numel(heads)
    ...
    for k = 16:20
        ...
        for j = 1:numel(latent_dimensions)
            if strcmp(heads{i},'vowel')
                options.epsilon = 1e-3;
            elseif strcmp(heads{i},'iris')
                options.epsilon = 5e-2;                
            elseif strcmp(heads{i},'halfcircles_N100K3')
                options.epsilon = 1e-3;                
            elseif strcmp(heads{i},'halfcircles2_N100K3')
                options.epsilon = 5e-3;                
            elseif strcmp(heads{i},'glass') && latent_dimensions{j} == 9
                options.epsilon = 1e-3;
            elseif strcmp(heads{i},'wine') && latent_dimensions{j} == 13
                options.epsilon = 5e-3;
            else
                options.epsilon = 1e-2;
            end
            fprintf('%s %d %f\n',heads{i},latent_dimensions{j},options.epsilon);
