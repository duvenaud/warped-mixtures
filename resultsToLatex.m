...
function resultsToLatex4(filename, results, methodNames, metricNames, ...
  experimentName,lowBetter,outputStd,transpose)


meanfunc = @mean;
stdfunc = @std;
%results = permute(results, [ 3 2 1 ] );
results = permute(results, [ 2 3 1 ] );
%results: experiment idx, method, metric
...
for ii = 1:methods  
    for jj = 1:metrics
        % run a paired ttest
        %if abs(meanScore(ii,jj) - best(jj)) <= stds(ii,jj) +
        %stds(best_ix(jj), jj)
...
for ii = 1:metrics
  ...
  orderMagBest = (2+ceil(log10(max(min(meanScore(:, ii)), 0))))^10;
...
for ii = 1:methods
  ...
  for jj = 1:metrics
    if nearbest(ii, jj)
        if outputStd == 1
            fprintf(file, [' & $\\mathbf{' printFormat '} \\pm ' printFormat '$'], ...
                meanScore(ii, jj),stds(ii,jj));
        else
    ...
    elseif meanScore(ii, jj) > maxClipCol(jj)
        ...
        if outputStd == 1
            fprintf(file, [' & $' printFormat ' \\pm ' printFormat '$' ], ...
                meanScore(ii, jj),stds(ii,jj));
        else
            fprintf(file, [' & $' printFormat '$' ],meanScore(ii,jj));
        end
    end
  end
  fprintf(file, ' \\\\\n');
